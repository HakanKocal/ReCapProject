using System;
using System.Collections.Generic;
using System.Text;

namespace Business.Constans
{
    public class Messages
    {
        public static string CarAdded = "Araba Başarıyla Eklendi";
        public static string CarNameInvalid = "Araba ismi 2 kelimeden büyük ve günlük fiyatı 0 dan büyük olmalıdır";
        public static string CarDeleted = "Araba Başarıyla Silindi";
        public static string CarUpdated = "Araba Başarıyla Güncellendi";
        public static string CarListed = "Arabalar Listelendi";
        public static string CarDetailsListed = "Araba Detayları Listelendi";

        public static string BrandAdded = "Brand Başarıyla Eklendi";
        public static string BrandDeleted = "Brand Başarıyla Silindi";
        public static string BrandUpdated = "Brand Başarıyla Güncellendi";
        public static string BrandListed = "Brand Listelendi";


        public static string ColorAdded = "Color Başarıyla Eklendi";
        public static string ColorDeleted = "Color Başarıyla Silindi";
        public static string ColorUpdated = "Color Başarıyla Güncellendi";
        public static string ColorListed = "Color Listelendi";


        public static string UserAdded = "User Başarıyla Eklendi";
        public static string UserDeleted = "User Başarıyla Silindi";
        public static string UserUpdated = "User Başarıyla Güncellendi";
        public static string UserListed = "User Listelendi";

        public static string RentalAdded = "Rental Başarıyla Eklendi";
        public static string RentalDeleted = "Rental Başarıyla Silindi";
        public static string RentalUpdated = "Rental Başarıyla Güncellendi";
        public static string RentalListed = "Rental Listelendi";
        public static string RentalCarNotAvaible = "Araba Baskası tarafından Suan Kullanımda ";


        public static string CustomerAdded = "Customer Başarıyla Eklendi";
        public static string CustomerDeleted = "Customer Başarıyla Silindi";
        public static string CustomerUpdated = "Customer Başarıyla Güncellendi";
        public static string CustomerListed = "Customer Listelendi";

        public static string ImageDeleted = "Resim Başarıyla Silindi";

        public static string ImageLimitExceded = "Limit Aşıldı";

        public static string ImageUpdate = "Resim Başarıyla Güncellendi";
        public static string ImageAdded = "Resim Başarıyla EKlendi";
    }
}
